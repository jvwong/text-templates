...
const handleResponse = res => {
  ...
  console.info( `Email sent to: ${info.accepted}; ${info.previewUrl ? 'Caught by Ethereal: ' + info.previewUrl: ''}` );
  return info;
};

const handleErr = err => {
  console.error( `Error encountered in email delivery: ${err.message}` );
  throw err;
};
...
const getSend = async () => {
  ...
  if( !EMAIL_ENABLED ) {
    console.info( `Mocking email transport` );
